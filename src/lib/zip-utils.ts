...
class SimpleZip {
  ...
  async addFromUrl(url: string, filename: string) {
    try {
      ...
      console.log(
        `Successfully downloaded: ${filename} (${arrayBuffer.byteLength} bytes)`
      );
    } catch (error) {
...
export async function createZipFromImages(
  ...
  for (let index = 0; index < imageUrls.length; index++) {
    ...
    try {
      ...
      console.log(
        `Downloading image ${index + 1}/${imageUrls.length}: ${filename}`
      );
  ...
  console.log(
    `ZIP creation completed: ${successCount} successful, ${errors.length} failed`
  );
